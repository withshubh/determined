...
func saveWorkload(db *db.PgDB, w workload.Workload) error {
	...
	case workload.CheckpointModel:
		return db.AddCheckpoint(model.NewCheckpoint(w.TrialID, w.TotalBatch()))
	case workload.ComputeValidationMetrics:
		return db.AddValidation(model.NewValidation(w.TrialID, w.TotalBatch()))
	default:
...
func markWorkloadErrored(db *db.PgDB, w workload.Workload) error {
	switch w.Kind {
	case workload.RunStep:
		return db.UpdateStep(w.TrialID, w.TotalBatch(), model.ErrorState, nil)
	case workload.CheckpointModel:
		return db.UpdateCheckpoint(w.TrialID, w.TotalBatch(), model.Checkpoint{State: model.ErrorState})
	case workload.ComputeValidationMetrics:
		return db.UpdateValidation(w.TrialID, w.TotalBatch(), model.ErrorState, nil)
	default:
...
func markWorkloadCompleted(db *db.PgDB, msg workload.CompletedMessage) error {
	...
	case workload.RunStep:
		return db.UpdateStep(
			msg.Workload.TrialID, msg.Workload.TotalBatch(), model.CompletedState, msg.RunMetrics)
	case workload.CheckpointModel:
		...
		return db.UpdateCheckpoint(
			msg.Workload.TrialID, msg.Workload.TotalBatch(), checkpoint)
	case workload.ComputeValidationMetrics:
		...
		return db.UpdateValidation(
			msg.Workload.TrialID, msg.Workload.TotalBatch(), model.CompletedState, metrics)
	default:
