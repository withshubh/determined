...
FROM trials t
INNER JOIN steps s ON t.id=s.trial_id
LEFT OUTER JOIN validations v ON s.total_batch=v.total_batch AND s.trial_id=v.trial_id
WHERE t.experiment_id=$1
  AND v.end_time > $2
...
func (db *PgDB) TrainingMetricBatches(experimentID int, metricName string, startTime time.Time) (
	...
	err = db.queryRows(`
SELECT (s.total_batch) AS batches_processed,
  max(s.end_time) as end_time
...
SELECT t.id FROM (
  ...
  FROM trials t
    INNER JOIN steps s ON t.id=s.trial_id
    RIGHT JOIN validations v ON s.total_batch=v.total_batch AND s.trial_id=v.trial_id
  WHERE t.experiment_id=$2
    AND v.state = 'COMPLETED'
  GROUP BY t.id
...
FROM trials t
  ...
  INNER JOIN (
    SELECT
      t.id as trial_id,
	  s.total_batch AS total_batch,
	  max(s.total_batch) AS batches
    FROM trials t
  	INNER JOIN steps s ON t.id=s.trial_id
    WHERE t.experiment_id=$2
	  AND s.state = 'COMPLETED'
	...
	ON s.total_batch = filter.total_batch
