...
c AS (
  ...
  FROM checkpoints c
  JOIN steps s ON c.total_batch = s.total_batch AND c.trial_id = s.trial_id
  LEFT JOIN validations v ON v.total_batch = s.total_batch AND v.trial_id = s.trial_id
